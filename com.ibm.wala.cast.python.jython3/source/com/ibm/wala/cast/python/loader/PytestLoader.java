...
public class PytestLoader extends Python3Loader {
	...
	public static final TypeReference pytestType = TypeReference.findOrCreate(PythonTypes.pythonLoader, "LPytest");
	...
	protected TranslatorToIR initTranslator(Set<Pair<CAstEntity, ModuleEntry>> topLevelEntities) {
		return new PythonCAstToIRTranslator(this) {
			...
			@Override
			protected void leaveTypeEntity(CAstEntity arg0, WalkContext arg1, WalkContext arg2,
					CAstVisitor<WalkContext> arg3) {
				...
				for(CAstEntity field : arg0.getAllScopedEntities().get(null)) {
		   			...
		   			if (field.getKind() == CAstEntity.FUNCTION_ENTITY) {
			    		...
			    		code.cfg().addInstruction(Python.instructionFactory().GetInstruction(code.cfg().getCurrentInstruction(), method, v, fr));
			...
			@Override
			protected void doMaterializeFunction(CAstNode node, WalkContext context, int result, int exception,
					CAstEntity fn) {
			...
			private void handlePytest(WalkContext context, CAstEntity fn, int function) {
				...
				if (isPytestEntry(fn)) {
					...
					code.cfg().addInstruction(
						new PythonInvokeInstruction(pos, context.currentScope().allocateTempValue(), context.currentScope().allocateTempValue(), site, new int[] { function }, keys));
